...
function beautify(sourceFile, options) {
    ...
    var filepath = path.resolve('.', sourceFile);
    ...
    try {
        ...
        switch(options.fileType) {
            case 'htm':
            case 'html':
                output = require('./format-html.js')(data, options);
                break;
            case 'json':
                output = JSON.stringify(JSON.parse(data), null, (options.indent || 4));
                break;
            case 'js':
                output = require('./format-js.js')(data, options);
                break;
            case 'css':
                output = require('./format-css.js')(data, options);
                break;
            default:
    ...
    edp.log.info('Success beautify file to [' + outputFile + '].');
}
